  ...
  methods: {
    ...
    [CRUD.HOOK.afterValidateCU](crud) {
      if (!crud.form.deptId) {
        ...
        return false
      } else if (crud.form.jobIds.length === 0) {
        this.$message({
          message: '岗位不能为空',
          type: 'warning'
        })
        return false
      } else if (crud.form.roleIds.length === 0) {
        this.$message({
          message: '角色不能为空',
          type: 'warning'
        })
    ...
    getDepts() {
      getDepts({enabled: true}).then(res => {
        this.depts = res.results.map(function (obj) {
          if (obj.hasChildren) {
            obj.children = null
          }
          return obj
    ...
    loadDepts({action, parentNode, callback}) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        getDepts({enabled: true, pid: parentNode.id}).then(res => {
          parentNode.children = res.results.map(function (obj) {
            if (obj.hasChildren) {
              obj.children = null
            }
            return obj
